...
CSession::CSession(boost::asio::io_context& io_context, CServer* server):
	_socket(io_context), _server(server), _b_close(false),_b_head_parse(false), _user_uid(0), m_timer(_socket.get_executor()){
...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			std::cout << "receive node data:" << _recv_msg_node->_data << std::endl;
...
void CSession::handleDisconnect(const boost::system::error_code& ec){
...
void CSession::sendHeartBeat(){
	auto self(shared_from_this());
	boost::asio::async_write(_socket, boost::asio::buffer(HEART_BEAT), 
		[this, self](boost::system::error_code ec, std::size_t) {
			if (ec) {
				std::cout << "发送失败，将断开连接" << std::endl;
				handleDisconnect(ec);
			}
		});
}

void CSession::startHeartBeat(){
	auto self(shared_from_this());
